...
public:
    ...
    {
        cout << "You are running Fermat_Count_IDP_CNTPM version. Prime for ID: " << PRIME_ID << endl;
        create_array();
        // hash
        if (use_fing)
            hash_fp = new BOBHash32(_init);
    ...
    void Insert(uint32_t flow_id, uint32_t cnt) override
    {
        ...
        else
        {
            for (int i = 0; i < array_num; ++i)
            {
                ...
                // printf("Before judging in %d\n", __LINE__);
                // cout << "Inserted flow_id: " << flow_id << " ";
                id[i][pos] = ((uint64_t)id[i][pos] + (uint64_t)mulMod32(flow_id_, cnt, PRIME_ID)) % (uint64_t)PRIME_ID;
                if(sign > 0){
                    counter[i][pos] += cnt;
                }
                else{
