...
public:
    ...
    void increment(int idx, int sign) {
        ...
        // if (counter_w < 8) {
        //     int32_t signBit = 1 << (counter_w - 1);
        //     bool isNegative = (value & signBit) != 0;
        //     bool sameSign = (sign == 1 && !isNegative) || (sign == -1 && isNegative);
...
public:
    ...
    {
        // cout << "Enter tower's insert" << endl;
        if (type == CM)
    ...
    {
        // cout << "Enter tower's insertCM" << endl;
        bool flag = false;
        idx[0] = hash[0].run(key, 4) % line[0].width;
        idx[1] = hash[1].run(key, 4) % line[1].width;
        ...
        if (val0 < mask1){ //threshold?
            // cout << "val0: " << val0 << " threshold: " << mask1 << endl;
            flag = true;
        }
        if (val1 < mask2){ //threshold?
            // cout << "val1: " << val1 << " threshold: " << mask2 << endl;   
            flag = true;
        }
        if (val0 != mask1)
