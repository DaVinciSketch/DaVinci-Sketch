...
{
    uint32_t fp;
	uint32_t checked_id = 50331651;
	bool printout = (*(uint32_t*)key == checked_id);
	if(printout){
		printf("******************Inserting %d******************\n", checked_id);
	}
    ...
    do{
		...
		for(int i = 0; i < COUNTER_PER_BUCKET - 1; i++){
			...
			if(buckets[pos].key[min_counter] == checked_id){
				printf("******************Find EXISTED and insert %d******************\n", checked_id);
			}
				matched = i;
				break;
			}
			if(buckets[pos].key[i] == 0 && empty == -1){
				if(printout){
					printf("Find empty in Heavy Part: %d - %d\n", pos, i);
				}
				empty = i;
			}
			if(min_counter_val > GetCounterVal(buckets[pos].val[i])){
				if(printout){
					printf("Find min in Heavy Part: %d - %d (%d)\n", pos, i, GetCounterVal(buckets[pos].val[i]));
				}
				min_counter = i;
		...
		if(empty != -1){
			...
			if(buckets[pos].key[min_counter] == checked_id){
				printf("******************Find empty and insert %d******************\n", checked_id);
			}
