...
public:
    ...
    void diff(FlowRadar &sketch2)
    {
        for (int i = 0; i < array_len; ++i) {
            uint32_t flow_id = flow_xor[i];
            uint32_t count1 = packet_count[i];
            uint32_t count2 = flow_count[i];
            sketch2.flow_xor[i] = flow_id ^ sketch2.flow_xor[i];
            sketch2.flow_count[i] = count2 - sketch2.flow_count[i];
            sketch2.packet_count[i] = count1 - sketch2.packet_count[i];
        }
    }
};
