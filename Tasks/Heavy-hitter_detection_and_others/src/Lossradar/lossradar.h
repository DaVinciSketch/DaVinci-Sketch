...
//#include "../ElasticRadar/common/BOBHash32.h"
...
public:
    ...
    uint32_t seq_memory_access_counter;
    ...
    void Insert_range_data(uint32_t id, uint16_t count) {
        ...
        for (uint16_t i = 0; i < count; ++i)
            Insert_id_seq(id, i);
            memory_access_counter += seq_memory_access_counter;
    }
    ...
    void diff(LossRadar &sketch2) {
        for (int i = 0; i < array_len; ++i) {
            uint32_t id = xorSum[i];
            uint16_t seq = xorNo[i];
            uint32_t count = countSum[i];

            sketch2.xorSum[i] = id ^ sketch2.xorSum[i];
            sketch2.xorNo[i] = seq ^ sketch2.xorNo[i];
            sketch2.countSum[i] = count - sketch2.countSum[i];
        }
    }
};
