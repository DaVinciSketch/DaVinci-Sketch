...
public:
	...
	uint32_t lp_memory_access_counter;
public:
	...
	{
		//printf("joinsketch insert!\n");
		int g = 0;
		lp_memory_access_counter = 0;
		for (int i = 0; i < d; i++)
		{
...
public:
	int n;
	const int m=8;
	uint64_t memory_access_counter;
	uint64_t id_memory_access_counter;
	struct data{
		unsigned Id,Key;
...
public:
	...
	uint64_t getMemoryAccessCounter() const override {
    	return memory_access_counter;
	}
	void Insert(const unsigned id){
		//printf("joinsketch insert!\n");
		id_memory_access_counter = 0;
		if(Heavy->count(id)){
			Heavy->insert(id);return;
			id_memory_access_counter += Heavy->hp_memory_access_counter;
		}
		unsigned k=MurmurHash32(&id, 4, hash_seed)%n;
		data*bucket=Mid[k];
		id_memory_access_counter++;
		for(int i=0;i<m;i++)
		...
		id_memory_access_counter += Light->lp_memory_access_counter;
		bucket[to].Key=1;
		bucket[to].Id=id;
		id_memory_access_counter++;
		return;
	...
	void Insert(const char* str)
	{
		...
		Insert(id);
		memory_access_counter += id_memory_access_counter;
		return;
	}
