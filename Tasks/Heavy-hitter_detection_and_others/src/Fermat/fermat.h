...
public:
    ...
    void Union(Fermat &sketch2)
    {     
        //counter[i][j] - sketch2.counter[i][j]
        for (int i = 0; i < array_num; i++)
        {
            for (int j = 0; j < entry_num; j++)
            {
                sketch2.id[i][j] = ((uint64_t)id[i][j] + (uint64_t)sketch2.id[i][j]) % PRIME_ID;
                sketch2.counter[i][j] += counter[i][j];
                if (use_fing)
                    sketch2.fingerprint[i][j] = ((uint64_t)fingerprint[i][j] + (uint64_t)sketch2.fingerprint[i][j]) % PRIME_FING;
            }
        }
        //sketch2.setArray(Arrays2);
    }

    void diff(Fermat &sketch2)
    {
        for (int i = 0; i < array_num; i++)
        {
            for (int j = 0; j < entry_num; j++)
            {
                sketch2.id[i][j] = (PRIME_ID + (uint64_t)id[i][j] - (uint64_t)sketch2.id[i][j]) % PRIME_ID;
                sketch2.counter[i][j] = (counter[i][j] - sketch2.counter[i][j]);// % PRIME_ID;
            }
        }
    }
};
