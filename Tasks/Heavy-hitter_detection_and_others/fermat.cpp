    ...
    for (int times = 0; times < TIMES; times++)
    {
        ...
        for (int iter_data = 0; iter_data < allwindowscnt; ++iter_data)
        {
            ...
            for (int i = 0; i < num_pkt; ++i)
            {
                ...
                fermat_tower->insert((const char *)(traces[traceindex][i].key), 1);
            }
            ...
            // // 首先处理只存在于一个字典中的键
            // for (const auto& elem : fermat_tower->Eleresult) {
            //     if (fermat_tower->EleFermatInserted.find(elem.first) == fermat_tower->EleFermatInserted.end()) {
            //         // 如果键仅存在于 Eleresult 中
            //         outFile << elem.first << "," << elem.second << ",\n";
            //     }
            // }

            // for (const auto& elem : fermat_tower->EleFermatInserted) {
            //     if (fermat_tower->Eleresult.find(elem.first) == fermat_tower->Eleresult.end()) {
            ...
            // // 然后处理两个字典中都有的键
            // for (const auto& elem : fermat_tower->Eleresult) {
            //     auto it = fermat_tower->EleFermatInserted.find(elem.first);
            //     if (it != fermat_tower->EleFermatInserted.end()) {
            //         // 如果键同时存在于两个字典中
            ...
            if (iter_data)
            {
                ...
                {
                    cout << sketches[iter_data]->query((const char *)&f.first) - sketches[iter_data - 1]->query((const char *)&f.first) << ",";  
                ...
                {
                    cout << sketches[iter_data - 1]->query((const char *)&f.first) - sketches[iter_data]->query((const char *)&f.first) << ",";  
