...
#include <queue>
#include <cstring>
#include "../Common/CommonBOBHash32.h"
#include "util/mod.h"
...
class Fermat
{
    ...
    int decodeflag = 0;
    // hash
    CommonBOBHash32 *hash;
    CommonBOBHash32 *hash_fp;
...
public:
    ...
    IdCountArrays getArrays() {
        IdCountArrays arrays;
        arrays.idArray = id;
        arrays.countArray = counter;
        //arrays.fingArray = fingerprint;
        return arrays;
    }

    void setArray(IdCountArrays arrays) {
        for (int i = 0; i < array_num; i++)
        {
            for (int j = 0; j < entry_num; j++)
            {
                id[i][j] = arrays.idArray[i][j];
                counter[i][j] = arrays.countArray[i][j];
                //fingerprint[i][j] = arrays.fingArray[i][j];
            }
        }
    }
    ...
    Fermat(int _a, int _e, bool _fing, uint32_t _init) : array_num(_a), entry_num(_e), use_fing(_fing), fingerprint(nullptr), hash_fp(nullptr)
    {
        ...
        // hash
        if (use_fing)
            hash_fp = new CommonBOBHash32(_init);
        hash = new CommonBOBHash32[array_num];
        for (int i = 0; i < array_num; ++i)
        {
