...
// void runSketchExperiments(std::function<void(int, bool, int, int, bool, int, const std::string&)> run_func, const std::string& taskName) {
...
void run_elastic(int _num_heavy_bucket = BUCKET_NUM, int _light_mem_byte = TOT_MEM_IN_BYTES - BUCKET_NUM*8*8, int endfile = 0, const std::string &str = NULL){
	...
	for(int datafileCnt = 0; datafileCnt <= endfile; ++datafileCnt)
	{
		...
		{
			elastic->insert((uint8_t*)(traces[datafileCnt][i].key));
		...
		delete elastic;
		Real_Freq.clear();
		outfile << total_packet_nums[endfile] << "," << total_flow_nums[endfile] << ",";
		outfile << "ElasticSketch,-,-,-,1," << _light_mem_byte << "," << _light_mem_byte << ",-,-," << ARE << "," << insert_duration << ",," << query_duration << endl;
	// }

	taskno++;
}
void run_flc(int _fermatcount = 2, bool _addundecodec = 1, int array_num = 3, int entry_num = (TOT_MEM_IN_BYTES - HEAVY_MEM)/(3 * 8), bool _fing = 0, int endfile = 0, const std::string &str = NULL){
	static constexpr int bucket_num = BUCKET_NUM;
	...
	for(int datafileCnt = 0; datafileCnt <= endfile; ++datafileCnt)
	{
		...
		for(int i = 0; i < packet_cnt; ++i)
		{
			flcsketch->insert(reinterpret_cast<const char*>(traces[datafileCnt][i].key));
			string str((const char*)(traces[datafileCnt][i].key), 4);
			Real_Freq[str]++;
		}
	}
		std::cout<<"Real_Freq.size()"<<Real_Freq.size()<<std::endl;
		// printf("After Insert\n");
		...
		auto end_query = std::chrono::high_resolution_clock::now();
				...
				std::cout << "Decoded correct num: " << decode_cor << " Decoded false num: " << decode_fal << std::endl;
				

				if(!flag){
					std::cout<<"These two unordered maps are always the same!\n";
				}
				outFile.close();
		#endif
...
int main()
{
	...
	for(int endfile = 0; endfile < END_FILE_NO; endfile++){
		...
		for(int i = 1;i<=20;i++){
			run_flc(2, true, 3, i*(TOT_MEM_IN_BYTES - HEAVY_MEM), false, endfile, "No finger 3 arrays and Same Entry Num per Array"); //Same single array's num of entry
			...
			run_elastic(BUCKET_NUM, i*(TOT_MEM_IN_BYTES - HEAVY_MEM), endfile, "Original Elastic Sketch"); //Original Elastic Sketch
			
		}
		for(int i = -2;i<3;i++){
			run_flc(2, true, 3, (int)((pow(10, i))*(TOT_MEM_IN_BYTES - HEAVY_MEM)), false, endfile, "No finger 3 arrays and Entry Num per Array"); //Same single array's num of entry
			...
			run_elastic(BUCKET_NUM, (int)((pow(10, i))*(TOT_MEM_IN_BYTES - HEAVY_MEM)), endfile, "Original Elastic Sketch"); //Original Elastic Sketch
		}
		for(int i = 1;i<=20;i++){
			run_flc(2, true, 3, i*(TOT_MEM_IN_BYTES - HEAVY_MEM)/(3*8), false, endfile, "No finger 3 arrays and Same Mem"); //Same single array's num of entry
		...
		for(int i = -2;i<3;i++){
			run_flc(2, true, 3, (int)((pow(10, i))*(TOT_MEM_IN_BYTES - HEAVY_MEM)/(3*8)), false, endfile, "No finger 3 arrays and Same Mem"); //Same single array's num of entry
