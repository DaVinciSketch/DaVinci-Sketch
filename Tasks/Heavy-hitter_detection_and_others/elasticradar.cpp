...
int main()
{
    ...
    {
        flcsketch = new FLCSketch<bucket_num>();
        true_freqs[0].clear();
        int allwindowscnt = 1;
        for (int iter_data = 0; iter_data < allwindowscnt; ++iter_data)
        {
            ...
            int traceindex = iter_data;
            int num_pkt = (int)traces[traceindex].size();
            printf("num_pkt: %d\n", num_pkt);
            for (int i = 0; i < num_pkt; ++i)
            {
                ++true_freq[*((uint32_t *)(traces[traceindex][i].key))];
                flcsketch->insert((const char *)(traces[traceindex][i].key), 1);
            }
            cout << "insertion done, flow size is" << true_freq.size() << endl;

            std::map<int, int, CompareByAbs> counter_mapping;

            // 假设array_num是3，entry_num是数组的长度
            int array_num = 3;
            int entry_num = flcsketch->fermatEle->get_entry_num(); // 你需要提供一个获取entry_num的方法

            // 遍历三个数组
            for (int n_array = 0; n_array < array_num; ++n_array) {
                for (int n = 0; n < entry_num; ++n) {
                    // 获取counter的值
                    int counter_value = flcsketch->fermatEle->get_counter(n_array, n);
                    // 更新字典
            ...
            {
                double dist = abs((int)it->second - (int)flcsketch->query((const char *)&(it->first), 1));
            ...
            //     uint32_t est_val = flcsketch->query((const char *)key);
            ...
            // flcsketch->get_heavy_hitters(fermattower_hh);
            ...
            // {
            //     uint32_t key = *itr;
            //     HH_are += std::abs((double)((int)true_freq[key] - (int)flcsketch->query((const char *)&key))) / (double)true_freq[key];
            // }
            // HH_are /= HH_true.size();
            ...
            // // int card = flcsketch->get_cardinality();
            ...
            //     double WMRD_nom = 0;
            //     double WMRD_denom = 0;
            //     flcsketch->get_distribution(dist);
            //     printf("get ok\n");
            //     fflush(stdout);
